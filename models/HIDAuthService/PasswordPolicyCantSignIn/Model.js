...
define([], function() {
    ...
    var objectMetadata;
    var context = {"object" : "PasswordPolicyCantSignIn", "objectService" : "HIDAuthService"};
    ...
    function PasswordPolicyCantSignIn(defaultValues) {
        var privateState = {};
        context["field"] = "maxLength";
        context["metadata"] = (objectMetadata ? objectMetadata["maxLength"] : null);
    ...
    BaseModel.isParentOf(PasswordPolicyCantSignIn);

    //Create new class level validator object
    BaseModel.Validator.call(PasswordPolicyCantSignIn);

    var registerValidatorBackup = PasswordPolicyCantSignIn.registerValidator;

    PasswordPolicyCantSignIn.registerValidator = function() {
        var propName = arguments[0];
        if(!setterFunctions[propName].changed) {
            var setterBackup = setterFunctions[propName];
            setterFunctions[arguments[0]] = function() {
                if(PasswordPolicyCantSignIn.isValid(this, propName, val)) {
    ...
    //For Operation 'getPasswordPolicy' with service id 'getPasswordPolicy6237'
     PasswordPolicyCantSignIn.getPasswordPolicy = function(params, onCompletion){
        return PasswordPolicyCantSignIn.customVerb('getPasswordPolicy', params, onCompletion);
     };

    var relations = [];

    PasswordPolicyCantSignIn.relations = relations;

    PasswordPolicyCantSignIn.prototype.isValid = function() {
        return PasswordPolicyCantSignIn.isValid(this);
    };

    PasswordPolicyCantSignIn.prototype.objModelName = "PasswordPolicyCantSignIn";
    PasswordPolicyCantSignIn.prototype.objServiceName = "HIDAuthService";
     ...
     */
    PasswordPolicyCantSignIn.registerProcessors = function(options, successCallback, failureCallback) {
        ...
        kony.mvc.util.ProcessorUtils.getMetadataForObject("HIDAuthService", "PasswordPolicyCantSignIn", options, metaDataSuccess, metaDataFailure);
    };

    //clone the object provided in argument.
    PasswordPolicyCantSignIn.clone = function(objectToClone) {
        var clonedObj = new PasswordPolicyCantSignIn();
        clonedObj.fromJsonInternal(objectToClone.toJsonInternal());
        return clonedObj;
    };

    return PasswordPolicyCantSignIn;
});