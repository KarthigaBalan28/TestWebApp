...
define([], function() {
    ...
    var objectMetadata;
    var context = {"object" : "GetUserAttributes", "objectService" : "HIDUserManagement"};
    ...
    function GetUserAttributes(defaultValues) {
        var privateState = {};
        context["field"] = "username";
        context["metadata"] = (objectMetadata ? objectMetadata["username"] : null);
    ...
    BaseModel.isParentOf(GetUserAttributes);

    //Create new class level validator object
    BaseModel.Validator.call(GetUserAttributes);

    var registerValidatorBackup = GetUserAttributes.registerValidator;

    GetUserAttributes.registerValidator = function() {
        var propName = arguments[0];
        if(!setterFunctions[propName].changed) {
            var setterBackup = setterFunctions[propName];
            setterFunctions[arguments[0]] = function() {
                if(GetUserAttributes.isValid(this, propName, val)) {
    ...
    //For Operation 'getUserAttributes' with service id 'getUserAttributes2283'
     GetUserAttributes.getUserAttributes = function(params, onCompletion){
        return GetUserAttributes.customVerb('getUserAttributes', params, onCompletion);
     };

    var relations = [];

    GetUserAttributes.relations = relations;

    GetUserAttributes.prototype.isValid = function() {
        return GetUserAttributes.isValid(this);
    };

    GetUserAttributes.prototype.objModelName = "GetUserAttributes";
    GetUserAttributes.prototype.objServiceName = "HIDUserManagement";
     ...
     */
    GetUserAttributes.registerProcessors = function(options, successCallback, failureCallback) {
        ...
        kony.mvc.util.ProcessorUtils.getMetadataForObject("HIDUserManagement", "GetUserAttributes", options, metaDataSuccess, metaDataFailure);
    };

    //clone the object provided in argument.
    GetUserAttributes.clone = function(objectToClone) {
        var clonedObj = new GetUserAttributes();
