define(function () {
  ...
  UserManagementBusinessController.prototype.searchUserAttributes = function(params, S_CB, F_CB){
    try{
      var getAttributesModel = ObjectServices.getDataModel("SearchUserAttributes");
      const callback = (status, response) => {
        if(status){
          S_CB(response.SearchUserAttributes[0]);
        } else {
          F_CB(response);
        }
      };
      getAttributesModel.customVerb("searchUserAttributes", params, callback);

    } catch(exception){
      alert("Exception occurred in getUserAttributes, error : "+ exception.message);
    }
  };

  UserManagementBusinessController.prototype.updateAttributes = function(params, S_CB, F_CB){
    try{
      var updateAttributesModel = ObjectServices.getDataModel("UpdateUserAttributes");
      const callback = (status,response) => {
        if(status){
          S_CB(response.UpdateUserAttributes[0]);
        } else {
          F_CB(response);
        }
      };
      updateAttributesModel.customVerb("updateUserAttributes", params, callback);
    } catch(exception){
      alert("Exception occurred in updateAttributes, error : "+ exception.message);
    }
  };
  UserManagementBusinessController.prototype.getUserAttributes = function(params, S_CB, F_CB){
    try{
      var getAttributesModel = ObjectServices.getDataModel("GetUserAttributes");
      const callback = (status, response) => {
        if(status){
          S_CB(response.GetUserAttributes[0]);
        } else {
          F_CB(response);
        }
      };
      getAttributesModel.customVerb("getUserAttributes", params, callback);
