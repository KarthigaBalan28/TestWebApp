define([`com/hid/onboarding/OnboardingBusinessController`], function(OnboardingBusinessController) {
  ...
  OnboardingPresentationController.prototype.addPasswordToUser = function(updateOnboardingUI,password,correlationId,factor){
      let Success_CB = success => {
      let UIObject = {
        "state" : "addPasswordToUserSuccess",//"addOOBToUserSuccess",
        "response" : {
          "response" : success
        }
      };
  ...
  OnboardingPresentationController.prototype.addOOBToUser = function(updateOnboardingUI,authType,correlationId,factor=null,userId = this.userId){
 ...
 //     this.sendOOB(updateOnboardingUI,authenticatorType);
  ...
  OnboardingPresentationController.prototype.initiateDeviceRegistrationPolling = function(updateOnboardingUI,did){
    ...
    let Success_CB = success => {
      ...
      kony.print("initiateDeviceRegistrationPolling: success: "+JSON.stringify(success));
      this.deviceAuthKey = success.DeviceRegistrationPolling[0].auth_key;
      let UIObject = {
        "state" : "deviceRegistrationPollingSuccess",
        "response" : {
          "response" : success.DeviceRegistrationPolling[0]
        }
      };
      updateOnboardingUI(UIObject);
      pollCounter = 2;
  ...
  OnboardingPresentationController.prototype.getDeviceOnboarding = function(updateOnboardingUI,correlationId,deviceId=this.deviceId,deviceAuthKey=this.deviceAuthKey){
        let S_CB = (success) => {
            let response = success.GetDevice[0];
            let UIObject = {
            "state" : "getDeviceOnboardingSuccess",
            "response" : {
            "response" : response
                }
            };
        ...
        let F_CB = (failure) => {
            let UIObject = {
                "state" : "getDeviceOnboardingFailure",
                "response" : {
                "response" : failure
                }
            };
        ...
        OnboardingBusinessController.getDevice(params,S_CB,F_CB);
    },

    OnboardingPresentationController.prototype.updateDeviceFriendlyNameOnboarding = function(updateOnboardingUI,correlationId,updatedName){
        let S_CB = (success) => {
            let UIObject = {
                "state" : "updateDeviceFriendlyNameOnboardingSuccess",
                "response" : {
                    response : success
                }
            }
        ...
        let F_CB = (failure) => {
            let UIObject = {
                "state" : "updateDeviceFriendlyNameOnboardingFailure",
                "response" : {
                    response : failure
                }
            }
        ...
        let params = {
            ...
            correlationId : correlationId,
            auth_key : this.deviceAuthKey,
            deviceId : this.deviceId

        }


        OnboardingBusinessController.updateFriendlyName(params,S_CB,F_CB)
