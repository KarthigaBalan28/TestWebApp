define([], function() {
  ...
  txnSigningBusinessController.prototype.approveStatusPolling = function(params, S_CB, F_CB) {
   ...
   txnSigningBusinessController.prototype.sendSMSOTP = function(params, S_CB, F_CB) {
    let objService = HIDAuthObjectServices.getDataModel("OTPRequest");
    const callback = (status, response) => {
      if (status) {
        S_CB(response);
      } else {
    ...
    objService.customVerb("sendOTP", params, callback);
  };
  ...
  txnSigningBusinessController.prototype.getClientAppProperties = function()
   {
    ...
    deviceType = (clientProperties.HID_DEVICE_TYPE && clientProperties.HID_DEVICE_TYPE !== null) ? clientProperties.HID_DEVICE_TYPE : deviceType;
      }, function(){});
   };
